...
// Skills tab and an MCP tab. All data crosses the host over the plugin's own
// authenticated Fetch route under Connection's `/api` prefix, not over
// `ctx.remote.commands.execute`.
//
// The command path was the original transport, but every command invocation
// appends a `command/run` + `command/done` pair to the session log, and the
// chat renders that pair as a permanent `skill-mgr · {"skills":[…]}` row whose
// text is the command's whole JSON result. This page lists on mount and
// refreshes after every mutation, so one settings visit left several large JSON
// rows in the conversation. The route below writes no session events at all.
//
// Verified against `@deepseek-ai/dsh` 0.1.7-alpha.2 (cordis 4.0.4). The one
// breaking change that reached this bundle: `ctx.sessions.list` is now a pure
// catalog (its `current` field is gone — "view selection remains outside the
// Controller"), so the open session is read from the retained main view.
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region locale
		/**
		 * The host half's authenticated Fetch route. It sits below Connection's
		 * `/api` prefix, which owns the Host/Origin trust fence and the browser
		 * session cookie — a plain same-origin fetch therefore needs no extra
		 * credentials, and the host handler never runs unauthenticated.
		 */
		...
		//#region index
		/**
		 * Services required by the Settings registration. `connection` is the Web
		 * transport carrier; `remote`/`remote.commands` are NOT used — the command
		 * path would write `command/run` + `command/done` rows into the
		 * conversation (see the file header).
		 */
		const inject = ["slots", "locale", "connection", "sessions"];
		/**
		 * Read the session the main view is currently showing.
		 *
		 * `ctx.sessions.list` is the CATALOG only: since 0.1.7 the sessions domain
		 * dropped its `current` selection field ("view selection remains outside
		 * the Controller"), and the retained main view is what names the open
		 * session. `retainedBy.mainView` is the reference count the Workspace UI
		 * holds while a session is on stage, so a positive count on any row is the
		 * current session — the same read the shipped `ui-layout`, `ui-cordis`,
		 * `ui-workspace`, `ui-session`, `ui-agent-preset`, and `settings-general`
		 * bundles perform. `phase` gates the read so a catalog that has not
		 * finished its first pull reports "no session" instead of "none selected".
		 * @param sessions - the injected `ctx.sessions` face.
		 * @returns the current session summary, or undefined when none is on stage.
		 */
		...
		function apply(ctx) {
			...
			/**
			 * Call one host endpoint over the plugin's Settings route.
			 *
			 * The payload carries the session identity so the host can restore the
			 * same cwd and skill-layer scope the tool path uses: `sessionId` lets it
			 * resolve the live Agent, and `cwd` keeps project roots working for a
			 * session the host is not currently driving. Nothing here touches the
			 * session log.
			 */
			const run = async (endpoint, payload) => {
				...
				let response;
				try {
					response = await fetch(SETTINGS_PATH, {
						method: "POST",
				...
				try {
					...
					return { ok: false, error: "unparsable response" };
				}
				if (body?.ok !== true) return {
					ok: false,
					error: typeof body?.error?.message === "string" ? body.error.message : "request failed"
				};
				return { ok: true, data: body.value };
			};
			const skillsApi = {
				snapshot: () => run("skill.snapshot", {}),
				get: (name) => run("skill.get", { name }),
				create: (payload) => run("skill.create", payload),
				update: (payload) => run("skill.update", payload),
				remove: (payload) => run("skill.delete", payload)
			};
			const mcpApi = {
				snapshot: () => run("mcp.snapshot", {}),
				get: (server) => run("mcp.get", { server }),
				add: (payload) => run("mcp.add", payload),
				update: (payload) => run("mcp.update", payload),
				remove: (payload) => run("mcp.remove", payload),
				reload: (server) => run("mcp.reload", { server })
			};
			let tabsVersion = -1;
			let tabsRevision = -1;
			let tabs = [];
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				id: "skill-mcp",
				// After every shipped section: account(-10), general(0), models(10),
				// plugins(15), agent-presets(20). The previous 20 tied with
				// agent-presets, leaving the last two rows in registration order.
				order: 25,
				label: () => t("nav"),
				locale: NS,
